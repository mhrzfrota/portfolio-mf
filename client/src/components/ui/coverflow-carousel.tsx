...
/** Imperative handle so navigation can live outside the carousel. */
...
export interface CoverflowCarouselProps {
  ...
  cardWidth?: string;
  /** Card width divided by height. 1 is the square of the original design. */
  aspect?: number;
  ...
  /** Rendered inside every card, above the image. */
  renderOverlay?: (context: {
    slide: CoverflowSlide;
    index: number;
    isActive: boolean;
  }) => React.ReactNode;
  /** Fires on a click that was not a drag. Off-centre cards centre themselves
      first, so the parent only has to decide what a click on the active card
      means. */
  onCardClick?: (index: number, isActive: boolean) => void;
  onSelect?: (index: number) => void;
  apiRef?: React.RefObject<CoverflowApi | null>;
}
...
export function CoverflowCarousel({
  ...
  /** A drag ends in a click event too; this keeps a throw from opening a card. */
  ...
  const paint = React.useCallback(() => {
    ...
    cardRefs.current.forEach((card, index) => {
      ...
      // A card faded out on the far side must not swallow clicks.
      card.style.pointerEvents = edge > 0.2 ? "auto" : "none";
    });
  ...
  React.useImperativeHandle(
    apiRef,
    () => ({ next: () => nudge(1), prev: () => nudge(-1), goTo }),
    [goTo, nudge]
  );

  // Kept in a ref so a fresh callback from the parent never re-fires it.
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Sem captura ainda: capturar no pointerdown faz o Chrome redirecionar o
    // `click` para o frame, e aí nem o card nem o link dentro dele recebem o
    // clique. A captura entra só quando o dedo/mouse anda de verdade.
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Abaixo do limiar nada se move: é um clique tremido, não um arrasto.
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!draggedRef.current) return; // clique puro: quem resolve é o card
    // Let a flick carry, but never more than two cards.
    const carried = Math.max(-2, Math.min(2, drag.v * 0.18));
    settle(clamp(Math.round(posRef.current + carried)));
    // O `click` do arrasto vem logo depois deste evento e é engolido pela
    // trava; o timer a solta em seguida, senão o clique seguinte morria junto.
    window.setTimeout(() => {
      draggedRef.current = false;
    }, 0);
  };
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {/* pointer-events none no trilho: os cards recuam no eixo Z e ficam
              atrás do plano do pai, que então engolia todo clique. Cada card
              devolve o seu (e a moldura continua recebendo o arrasto). */}
          <div
