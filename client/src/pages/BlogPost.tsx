      ...
      <header className="mt-10 flex flex-col items-center text-center">
        <div className="flex flex-wrap justify-center gap-2">
          {post.tags.map(tag => (
            <span
              key={tag}
              className="rounded-full border border-border px-3 py-1 text-[12px] font-semibold uppercase tracking-wide text-[var(--accent-blue)]"
            >
              {tag}
            </span>
          ))}
                  ...
                  return (
                    <ul key={index} className="space-y-3">
                      {block.items.map(item => (
                        ...
                        >
                          <span className="absolute left-0 top-[0.7em] h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-[var(--accent-blue)]" />
                          {item}
                        </li>
                      ))}
                    </ul>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {relatedPosts.map(item => (
              <Link
                key={item.slug}
                href={`/blog/${item.slug}`}
                className="group flex flex-col rounded-2xl border border-border bg-card p-6 transition-colors hover:border-[var(--accent-blue)]/50"
              >
                <span className="text-[12px] font-medium text-muted-foreground">
                  {formatDate(item.date, lang)}
                </span>
                <h3 className="mt-2 text-[16px] font-bold leading-snug tracking-[-0.01em] text-foreground transition-colors group-hover:text-[var(--accent-blue)]">
                  {item.title}
                </h3>
                <span className="mt-4 flex items-center gap-1.5 text-[13px] font-semibold text-[var(--accent-blue)]">
